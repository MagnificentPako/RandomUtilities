...
local function e(j)
local x=setmetatable({},{__index=_ENV or getfenv()})if setfenv then setfenv(j,x)end;return j(x)or x end
local t=e(function(j,...)local x,z=os.queueEvent,coroutine.yield;local function _()x("sleep")if z()=="terminate"then
error("Terminated")end end;return
{dir=shell.dir,refreshYield=_,serialize=textutils.serialize}end)
local a=e(function(j,...)local function x(E)
if type(E)=="string"then return string.format("%q",E)else return tostring(E)end end
...
I=I.. (T.."\tMetatable = {").."\n"I=I..z(H,T.."\t\t",A,O)
I=I.. (T.."\t}").."\n"end end;I=I.. (T.."}").."\n"end else I=I.. (T..x(E)).."\n"end;return I end
local function _(E,T,A)if T==nil then T=true end;return z(E,A or"",{length=0},T)end;return _ end)
local o=e(function(j,...)local x=false;local function z(C,...)local M=term.isColor()
if M then term.setTextColor(C)end;print(...)
if M then term.setTextColor(colors.white)end end
local function _(C,M)
local F=term.isColor()if F then term.setTextColor(C)end;io.write(M)if F then
term.setTextColor(colors.white)end end;local function E(...)z(colors.green,...)end;local function T(...)
z(colors.red,...)end;local function A(C)if C~=nil then x=C end;return x end
local function O(...)if x then local C,M=pcall(function()
error("",4)end)_(colors.gray,M)
z(colors.lightGray,...)end end
local function I(...)
...
Y=V(Y)else Y=tostring(Y)end;if F then Y=" "..Y end
F=true;_(colors.lightGray,Y)end;print()end end
...
for W=1,F do if C[W]~=M[W]then return false end end;return true end
local U=print
return
{Print=U,PrintError=T,PrintSuccess=E,PrintColor=z,WriteColor=_,IsVerbose=A,Verbose=O,VerboseLog=I,EscapePattern=S,ParsePattern=R,CreateLookup=D,MatchTables=L}end)
...
setmetatable({options=L or{},fn=D,channel=nil,id=math.random(1000000000)},{__index=_})end;local T={}
local function A(D,L)return
setmetatable({stopped=false,namespace=D,callbacks={},channels={},parent=L},{__index=T})end
...
C=L.priority end
table.insert(self.callbacks,C,U)return U end
function T:getSubscriber(D)for U=1,#self.callbacks do local C=self.callbacks[U]if C.id==D then
return{index=U,value=C}end end;local L
for U,C in
z(self.channels)do L=C:getSubscriber(D)if L then break end end;return L end
function T:setPriority(D,L)local U=self:getSubscriber(D)
if U.value then
table.remove(self.callbacks,U.index)table.insert(self.callbacks,L,U.value)end end
function T:addChannel(D)self.channels[D]=A(D,self)return self.channels[D]end
function T:hasChannel(D)return D and self.channels[D]and true end;function T:getChannel(D)
return self.channels[D]or self:addChannel(D)end
function T:removeSubscriber(D)
local L=self:getSubscriber(D)
if L and L.value then
for U,C in z(self.channels)do C:removeSubscriber(D)end;return table.remove(self.callbacks,L.index)end end
...
D[#D]=M end;if C==false then return false,D end end end
if parent then return parent:publish(D,...)else return true,D end end;local O=A('root')
local function I(D)local O=O
if x(D)=="string"then if D:find(":")then
D={D:match((D:gsub("[^:]+:?","([^:]+):?")))}else D={D}end end;for L=1,#D do O=O:getChannel(D[L])end;return O end;local function N(D,L,U)return I(D):addSubscriber(L,U)end;local function S(D,L)return
I(L):getSubscriber(D)end;local function H(D,L)
return I(L):removeSubscriber(D)end
local function R(D,...)return I(D):publish({},...)end
return{GetChannel=I,Subscribe=N,GetSubscriber=S,RemoveSubscriber=H,Publish=R}end)
local n=e(function(j,...)
local x={__index=function(E,T)return
function(A,...)local O=A.parser;local I=O[T](O,A.name,...)if I==O then return A end;return I end end}local z={}
...
for S,H in ipairs(N)do O=A[H]if O~=nil then return O end end end;O=I.default;if O~=nil then return O end end;return T end;function z:Ensure(E)local T=self:Get(E)
if T==nil then error(E.." must be set")end;return T end;function z:Default(E,T)
...
if I==nil then O.aliases={T}else table.insert(I,T)end else A[E]={aliases={T}}end
self:_Changed()return self end
function z:Description(E,T)return self:_SetSetting(E,"description",T)end;function z:TakesValue(E,T)if T==nil then T=true end
return self:_SetSetting(E,"takesValue",T)end
function z:_SetSetting(E,T,A)local O=self.settings
local I=O[E]if I then I[T]=A else O[E]={[T]=A}end;return self end
function z:Option(E)return setmetatable({name=E,parser=self},x)end;function z:Arguments()return self.arguments end;function z:_Changed()
i.Publish({"ArgParse","changed"},self)end
function z:Help(E)
for T,A in pairs(self.settings)do local O='-'if A.takesValue then O="--"T=
T.."=value"end;if#T>1 then O='--'end;o.WriteColor(colors.white,E..
O..T)local I=""local N=A.aliases
if N and#N>0 then
local H=#N;I=I.." ("for R=1,H do local D="-"..N[R]if#D>2 then D="-"..D end
if R<H then D=D..', 'end;I=I..D end;I=I..")"end;o.WriteColor(colors.brown,I)local S=A.description;if S and S~=""then o.PrintColor(colors.lightGray,
" "..S)end end end
...
if H=="not-"or H=="not_"then S=false;I=I:sub(5)end;T[I]=S end else for N=2,#I do
T[I:sub(N,N)]=true end end else table.insert(A,I)end end;return self end
local function _(E)return
setmetatable({options={},arguments={},settings={}},{__index=z}):Parse(E)end;return{Parser=z,Options=_}end)
...
return O,E end end;if E=="/"or E==""then break end;E=fs.getDir(E)end;return nil,
"Cannot find HowlFile. Looking for '"..table.concat(T,"', '").."'"end;local z={}
local function _(E)
local T=setmetatable(E or{},{__index=getfenv()})T._G=_G
function T.loadfile(A)return setfenv(loadfile(A),T)end;function T.dofile(A)return T.loadfile(A)()end
i.Publish({"HowlFile","env"},T)return T end;return{FindHowl=x,SetupEnvironment=_,CurrentDirectory=""}end)
...
self.filesProduced[E]=true;I=A.Name;O=A.Pattern.From end
for S,H in
pairs(self.patternMapsCache)do if E:match(S)then self.filesProduced[E]=true;I=H.Name
O=E:gsub(S,H.Pattern.From)break end end
if I then local S=self:DoRequire(O,true)
if not S then if not T then
o.PrintError("Cannot find '"..O.."'")end;return false end;return self:Run(I,O,N)end;if fs.exists(fs.combine(s.CurrentDirectory,E))then
self.filesProduced[E]=true;return true end;if not T then
o.PrintError(
"Cannot find a task matching '"..E.."'")end;return false end;local function z(E,T)local A=#E;if#E~=#T then return false end
for O=1,A do if E[O]~=T[O]then return false end end;return true end
function x:Run(E,...)
local T=E
if type(E)=="string"then T=self.tasks[E]if not T then
o.PrintError("Cannot find a task called '"..E.."'")return false end elseif not T or not T.Run then
o.PrintError("Cannot call task as it has no 'Run' method")return false end;local A={...}local O=self.ran[T]
if not O then O={A}self.ran[T]=O else for I=1,#O do if z(A,O[I])then
return true end end;O[#O+1]=A end;t.refreshYield()return T:Run(self,...)end
function x:Start(E)local T
if E then T=self.tasks[E]else T=self.default;E="<default>"end;if not T then
o.PrintError("Cannot find a task called '"..E.."'")return false end;return self:Run(T)end
...
error(string.format("Both '%s' and '%s' produces '%s'",D,O,R))end;E[R]=O end end
local S=I.maps
if S then
for H,R in ipairs(S)do
local D=(R.Type=="Pattern"and T or A)local L=R.To;local U=D[L]if U then
error(string.format("Both '%s' and '%s' match '%s'",U,O,L))end;D[L]={Name=O,Pattern=R}end end end;return self end
local function _(E)return
setmetatable({ran={},filesProduced={},tasks=E.tasks,default=E.default,Traceback=E.Traceback,ShowTime=E.ShowTime},{__index=x}):BuildCache()end;return{Factory=_,Context=x}end)
local r=e(function(j,...)
local function x(T,A)local O=o.ParsePattern(T,true)local I=o.ParsePattern(A)
local N=O.Type
assert(N==I.Type,"Both from and to must be the same type "..N.." and "..O.Type)return{Type=N,From=O.Text,To=I.Text}end;local z={}
function z:Depends(T)
if type(T)=="table"then local A=self.dependencies;for O,I in ipairs(T)do
table.insert(A,I)end else table.insert(self.dependencies,T)end;return self end
function z:Requires(T)
if type(T)=="table"then local A=self.requires;for O,T in ipairs(T)do
table.insert(A,T)end else table.insert(self.requires,T)end;return self end
function z:Produces(T)
if type(T)=="table"then local A=self.produces;for O,T in ipairs(T)do
table.insert(A,T)end else table.insert(self.produces,T)end;return self end
function z:Maps(T,A)table.insert(self.maps,x(T,A))return self end;function z:Action(T)self.action=T;return self end;function z:Description(T)
self.description=T;return self end;function z:_RunAction(...)
return self.action(self,...)end
function z:Run(T,...)for A,O in ipairs(self.dependencies)do if not T:Run(O)then
return false end end
for A,O in
ipairs(self.requires)do if not T:DoRequire(O)then return false end end
...
table.insert(newArgs,tostring(R))end;O=" ("..
table.concat(newArgs,", ")..")"end
o.PrintColor(colors.cyan,"Running "..self.name..O)local I=os.clock()local N,S=true,nil
if T.Traceback then
xpcall(function()
self:_RunAction(unpack(A))end,function(H)
for R=5,15 do
local D,S=pcall(function()error("",R)end)if H:match("Howlfile")then break end;H=H.."\n  "..S end;S=H;N=false end)else N,S=pcall(self._RunAction,self,...)end
if N then o.PrintSuccess(self.name..": Success")else o.PrintError(
self.name..": Failure\n"..S)end;if T.ShowTime then
o.Print(" ","Took "..os.clock()-I.."s")end;return N end;return true end
local _={__index=function(T,A)local O=z[A]if O then return O end
...
I or{__index=z})end;return{Factory=E,Task=z,OptionTask=_}end)
local d=e(function(j,...)local x={}function x:Task(_)
return function(E,T)return self:AddTask(_,E,T)end end;function x:AddTask(_,E,T)return
self:InjectTask(r.Factory(_,E,T))end;function x:InjectTask(_,E)
self.tasks[E or _.name]=_;return _ end
...
error("Cannot find task ".._)end else
self.default=r.Factory("<default>",{},_)end;return self end;function x:Run(_)return self:RunMany({_})end
function x:RunMany(_)
local E=os.clock()local T;local A=h.Factory(self)if#_==0 then A:Start()else
for O,I in ipairs(_)do T=A:Start(I)end end;if A.ShowTime then
o.PrintColor(colors.orange,"Took "..
os.clock()-E.."s in total")end;return T end;local function z()
return setmetatable({tasks={},default=nil},{__index=x})end;return{Factory=z,Runner=x}end)
local l=e(function(j,...)local x={}
function x:Name(E)self.name=E;self:Alias(E)return self end;function x:Alias(E)self.alias=E;return self end
function x:Depends(E)if type(E)=="table"then for T,A in
ipairs(E)do self:Depends(A)end else
table.insert(self.dependencies,E)end;return self end
function x:Prerequisite(E)
if type(E)=="table"then
for T,A in ipairs(E)do self:Prerequisite(A)end else table.insert(self.dependencies,1,E)end;return self end
function x:Export(E)if E==nil then E=true end;self.shouldExport=E;return self end
function x:NoWrap(E)if E==nil then E=true end;self.noWrap=E;return self end;local z={}
local function _(E,T)return
setmetatable({mainFiles={},files={},path=E or s.CurrentDirectory,namespaces={},shouldExport=false,parent=T},{__index=z})end;function z:File(E)local T=self:_File(E)self.files[E]=T
i.Publish({"Dependencies","create"},self,T)return T end;function z:Resource(E)
local T=self:_File(E)T.type="Resource"self.files[E]=T
i.Publish({"Dependencies","create"},self,T)return T end
function z:_File(E)return
setmetatable({dependencies={},name=
nil,alias=nil,path=E,shouldExport=true,noWrap=false,type="File",parent=self},{__index=x})end
...
assert(T,"Cannot find a main file")T:Depends(E)return self end
function z:Prerequisite(E)local T=self.mainFiles[1]
assert(T,"Cannot find a main file")T:Prerequisite(E)return self end
function z:FindFile(E)local T=self.files;local A=T[E]if A then return A end;A=T[E..".lua"]
if A then return A end;for O,A in pairs(T)do if A.alias==E then return A end end;return nil end
...
if not S then error("Cannot find file "..N)end;T(S)end
coroutine.yield(O)end;local A=self.mainFiles;if#A==0 then A=self.files end
...
local O=_(fs.combine(self.path,T or""),self)self.namespaces[E]=O;A(O)return O end
function z:CloneDependencies(E)local T=setmetatable({},{__index=z})
for A,O in pairs(self)do T[A]=O end;T.mainFiles={}return T end;function z:Paths()local E,T=table.insert,{}
for A,O in pairs(self.files)do E(T,O.path)end;return T end
i.Subscribe({"HowlFile","env"},function(E)
E.Dependencies=_;E.Sources=_()end)return{File=x,Dependencies=z,Factory=_}end)
...
O.writeLine(x)end;O.writeLine(_)local I={}
...
O.writeLine("env["..
j("%q",H).."] = "..
(N.noWrap and"doFile"or"doWithResult").."("..S..")")else O.writeLine("doFile("..S..")")end end;if T.traceback then O.writeLine(z)end;O.close()end
function d.Runner:CreateBootstrap(E,T,A,O)
return
self:InjectTask(r.Factory(E,O,function(I)T:CreateBootstrap(A,I)end,r.OptionTask)):Description(
"Creates a 'dynamic' combination of files in '"..A.."')"):Produces(A):Requires(T:Paths())end end
...
if z.type=="Resource"then z:Verify(false)end end)
function l.File:Verify(x)if x==nil then x=true end;self.verify=x;return self end end
...
I=S+1 end;return H end;local function T(O,I)
return O:gsub("{{(.-)}}",function(N)return I[N]or""end)end
i.Subscribe({"Combiner","start"},function(O,I,N)if O.finalizer then
N.traceback=true end;if N.lineMapping then N.oldLine=0;N.line=0;N.lineToModule={}
N.moduleStarts={}end;if N.traceback then
I.write("local __program = function(...)")end end)local A=math.min
...
local U=D[I]if U then D[I]=A(H,U)else D[I]=H end;L[A(H,R)]=I end end)
...
nil else
i.Publish({"Combiner","include"},O,z,finalizerContents,N)end;if finalizerContents then
S[#S+1]=z;H.finalizer=finalizerContents end end
if N.lineMapping then S[#S+1]=x;local R=t.serialize
H.lineToModule=R(N.lineToModule)H.moduleStarts=R(N.moduleStarts)H.lastLine=N.line end;toReplace={}
for R,D in ipairs(S)do for L,U in pairs(D)do local C=toReplace[L]
if C then C=C.."\n"else C=""end;toReplace[L]=C..U end end;I.write(T(T(_,toReplace),H))end end)
function l.Dependencies:Finalizer(O)
local I=self:FindFile(O)or self:File(O)I.type="Finalizer"self.finalizer=I
i.Publish({"Dependencies","create"},self,I)return I end end
...
C(U,{},self,F,M,E)then L(M)end end
H={write=R,path=_}end
...
if not F then
N.close()error(W[#W]or"Unknown error")end;o.Verbose("Adding "..U)local Y=L.name
if L.type=="Main"then
R(M,L.alias or L.path)elseif L.type=="Resource"then local P=
assert(Y,"A name must be specified for resource "..L.path).."="
if not
L.shouldExport then P="local "..P elseif not A then D[#D+1]=Y;P="local "..P end;R(P..I(M),L.alias or L.path)elseif Y then
local P,V=x..'(function(_ENV, ...)','end)'if L.noWrap then P,V='(function(...)','end)()'end;local B=Y..'='..P
if not
L.shouldExport then B="local "..B elseif not A then D[#D+1]=Y;B="local "..B end;R(B)R(M,Y)R(V)else local P=not L.noWrap;if P then R("do")end
R(M,L.alias or L.path)if P then R('end')end end end
if#D>0 and#self.mainFiles==0 then local L={}for U,C in ipairs(D)do L[#L+1]=C..
"="..C..", "end;R("return {"..
table.concat(L).."}")end
j:getChannel("end"):publish({},self,H,E)N.close()end
function d.Runner:Combine(_,E,T,A)
return
self:InjectTask(r.Factory(_,A,function(O)E:Combiner(T,O)end,r.OptionTask)):Description(
"Combines files into '"..T.."'"):Produces(T):Requires(E:Paths())end end
do
function d.Runner:ListTasks(j,x)local z={}local _=0
for E,T in pairs(self.tasks)do local A=E:sub(1,1)
if x or
(A~="_"and A~=".")then local O=T.description or""local I=#E;if I>_ then _=I end;z[E]=O end end;_=_+2;j=j or""for E,T in pairs(z)do o.WriteColor(colors.white,j..E)
o.PrintColor(colors.lightGray,string.rep(" ",
_-#E)..T)end;return self end
function d.Runner:Clean(j,x,z)
return
self:AddTask(j,z,function()
o.Verbose("Emptying directory '"..x.."'")
fs.delete(fs.combine(s.CurrentDirectory,x))end):Description(
"Clean the '"..x.."' directory")end end
...
T={Scope=self,Name=E,IsGlobal=false,CanRename=true,References=1}self:AddLocal(T)return T end;function z:GetLocal(E)
for T,A in pairs(self.Locals)do if A.Name==E then return A end end
if self.Parent then return self.Parent:GetLocal(E)end end;function z:GetOldLocal(E)if
self.oldLocalNamesMap[E]then return self.oldLocalNamesMap[E]end;return
self:GetLocal(E)end
...
if not A and self.Parent then self.Parent:RenameLocal(E,T)end end
...
T={Scope=self,Name=E,IsGlobal=true,CanRename=true,References=1}self:AddGlobal(T)return T end;function z:GetGlobal(E)
for T,A in pairs(self.Globals)do if A.Name==E then return A end end
if self.Parent then return self.Parent:GetGlobal(E)end end
function z:GetOldGlobal(E)
if
self.oldGlobalNamesMap[E]then return self.oldGlobalNamesMap[E]end;return self:GetGlobal(E)end
function z:RenameGlobal(E,T)E=type(E)=='string'and E or E.Name
local A=false;local O=self:GetGlobal(E)if O then O.Name=T
self.oldGlobalNamesMap[E]=O;A=true end;if not A and self.Parent then
self.Parent:RenameGlobal(E,T)end end;function z:GetVariable(E)
return self:GetLocal(E)or self:GetGlobal(E)end
function z:GetOldVariable(E)return self:GetOldLocal(E)or
self:GetOldGlobal(E)end
function z:RenameVariable(E,T)
E=type(E)=='string'and E or E.Name;if self:GetLocal(E)then self:RenameLocal(E,T)else
self:RenameGlobal(E,T)end end;function z:GetAllVariables()
return self:getVars(true,self:getVars(true))end
function z:getVars(E,T)local T=T or{}
if E then for A,O in
pairs(self.Children)do O:getVars(true,T)end else for A,O in pairs(self.Locals)do
table.insert(T,O)end
for A,O in pairs(self.Globals)do table.insert(T,O)end
if self.Parent then self.Parent:getVars(false,T)end end;return T end
...
A:sub(U,U)..D;L=S(L/I)end;N=N+1 end end until not(x[D]or self:GetVariable(D))self:RenameLocal(R.Name,D)end end;function z:ToString()return'<Scope>'end
local function _(E)
local T=setmetatable({Parent=E,Locals={},Globals={},oldLocalNamesMap={},oldGlobalNamesMap={},Children={}},{__index=z})if E then table.insert(E.Children,T)end;return T end;return _ end)
local m=e(function(j,...)local x={}function x:Peek(z)local _=self.tokens;z=z or 0;return
_[math.min(#_,self.pointer+z)]end
function x:Get(z)
local _=self.tokens;local E=self.pointer;local T=_[E]self.pointer=math.min(E+1,#_)if z then
table.insert(z,T)end;return T end;function x:Is(z)return self:Peek().Type==z end;function x:Save()
...
return true else return nil end else self:Get(_)return E end else return nil end end
function x:ConsumeKeyword(z,_)local E=self:Peek()if E.Type=='Keyword'and E.Data==z then
self:Get(_)return true else return nil end end;function x:IsKeyword(z)local _=self:Peek()
return _.Type=='Keyword'and _.Data==z end
function x:IsSymbol(z)local _=self:Peek()return _.Type==
'Symbol'and _.Data==z end
...
(self.Data or'').." >"end;local R={__index=H}
...
U:sub(F+Q,F+Q)end;local function B(Q)local J=V()for X=1,#Q do
if J==Q:sub(X,X)then return P()end end end;local function G(Q)
error(">> :"..W..
":"..Y..": "..Q,0)end
...
"]` near <eof>.",3)end;local ea=true
if V()==']'then for eo=1,J do
if V(eo)~='='then ea=false end end;if V(J+1)~=']'then ea=false end else
if V()=='['then
local eo=true;for ei=1,J do if V(ei)~='='then eo=false;break end end;if
V(J+1)=='['and eo then X=X+1;for ei=1,(J+2)do P()end end end;ea=false end
if ea then X=X-1;if X==0 then break else for eo=1,J+2 do P()end end else P()end end;local ee=U:sub(Z,F-1)for ea=0,J+1 do P()end;local et=U:sub(Q,F-1)return ee,et else
...
P()P()J="#!"while V()~='\n'and V()~=''do J=J..P()end
table.insert(Q,S({Type='Comment',CommentType='Shebang',Data=J,Line=W,Char=Y},R))J=""end
...
table.insert(Q,S({Type='Comment',CommentType=X and'LongComment'or'Comment',Data=J,Line=W,Char=Y},R))J=""end
...
table.insert(Q,S({Type='Comment',CommentType=X and'LongComment'or'Comment',Data=J,Line=W,Char=Y},R))end;local Z=W;local ee=Y;local et=":"..W..":"..
Y..":> "local ea=V()local eo=nil
if ea==''then
eo={Type='Eof'}elseif _[ea]or z[ea]or ea=='_'then local ei=F
repeat P()ea=V()until not(
_[ea]or z[ea]or E[ea]or ea=='_')local en=U:sub(ei,F-1)if O[en]then eo={Type='Keyword',Data=en}else
eo={Type='Ident',Data=en}end elseif
E[ea]or(V()=='.'and E[V(1)])then local ei=F
if ea=='0'and V(1)=='x'then P()P()while A[V()]do P()end;if B('Pp')then B('+-')while
E[V()]do P()end end else while E[V()]do P()end;if B('.')then
while E[V()]do P()end end
if B('Ee')then B('+-')while E[V()]do P()end end end;eo={Type='Number',Data=U:sub(ei,F-1)}elseif
...
G("Unfinished string near <eof>")end end;local eh=U:sub(es,F-2)local er=U:sub(ei,F-1)
eo={Type='String',Data=er,Constant=eh}elseif ea=='['then local ei,en=K()if en then eo={Type='String',Data=en,Constant=ei}else P()
eo={Type='Symbol',Data='['}end elseif B('>=<')then if B('=')then
eo={Type='Symbol',Data=ea..'='}else eo={Type='Symbol',Data=ea}end elseif B('~')then if
B('=')then eo={Type='Symbol',Data='~='}else
G("Unexpected symbol `~` in source.",2)end elseif B('.')then if B('.')then if B('.')then
eo={Type='Symbol',Data='...'}else eo={Type='Symbol',Data='..'}end else
eo={Type='Symbol',Data='.'}end elseif B(':')then if B(':')then
eo={Type='Symbol',Data='::'}else eo={Type='Symbol',Data=':'}end elseif T[ea]then
P()eo={Type='Symbol',Data=ea}else local ei,en=K()if ei then
eo={Type='String',Data=en,Constant=ei}else
G("Unexpected Symbol `"..ea.."` in source.",2)end end;eo.LeadingWhite=Q;eo.Line=Z;eo.Char=ee;C[#C+1]=S(eo,R)
if eo.Type=='Eof'then break end end end
local M=setmetatable({tokens=C,savedPointers={},pointer=1},{__index=m})return M end
local function L(U)
local function C(Q)
local J=">> :"..U:Peek().Line..
":"..U:Peek().Char..": "..Q.."\n"local X=0
...
do local et=Z:sub(ee,ee)if et=='\t'then J=J..'    'else J=J..' 'end end;J=J.."   ^^^^"break end end end;error(J)end;local M,F,W,Y,P
local function V(Q,J)local X=c(Q)if not U:ConsumeSymbol('(',J)then
C("`(` expected.")end;local Z={}local ee=false
while
...
C("`)` expected.")end end elseif U:ConsumeSymbol('...',J)then ee=true;if not U:ConsumeSymbol(')',J)then
C("`...` must be the last argument of a function.")end;break else
C("Argument name or `...` expected")end end;local et=F(X)if not U:ConsumeKeyword('end',J)then
C("`end` expected after function body")end;return
{AstType='Function',Scope=X,Arguments=Z,Body=et,VarArg=ee,Tokens=J}end
...
if not Z then Z=Q:CreateGlobal(X.Data)else Z.References=Z.References+1 end else Z.References=Z.References+1 end;return{AstType='VarExpr',Name=X.Data,Variable=Z,Tokens=J}else
C("primary expression expected")end end
...
not U:ConsumeSymbol(']',Z)then C("`]` expected.")end
...
U:ConsumeSymbol(')',Z)then break else C("`)` Expected.")end end end;X={AstType='CallExpr',Base=X,Arguments=ee,Tokens=Z}elseif
...
X={AstType='TableCallExpr',Base=X,Arguments={ee},Tokens=Z}else break end end;return X end
...
C("`]` Expected")end;if not U:ConsumeSymbol('=',J)then
C("`=` Expected")end;local et=M(Q)
...
local et=M(Q)X[#X+1]={Type='Value',Value=et}end elseif U:ConsumeSymbol('}',J)then break else local ee=M(Q)X[#X+1]={Type='Value',Value=ee}end
if U:ConsumeSymbol(';',J)or U:ConsumeSymbol(',',J)then elseif
U:ConsumeSymbol('}',J)then break else C("`}` or table entry Expected")end end;return Z elseif U:ConsumeKeyword('function',J)then local X=V(Q,J)X.IsLocal=true;return X else return P(Q)end end;local B=8
...
local et={AstType='UnopExpr',Rhs=X,Op=ee,OperatorPrecedence=B,Tokens=Z}X=et else X=W(Q)end
while true do local Z=G[U:Peek().Data]
if Z and Z[1]>J then local ee={}
local et=U:Get(ee).Data;local ea=M(Q,Z[2])
local eo={AstType='BinopExpr',Lhs=X,Op=et,OperatorPrecedence=Z[1],Rhs=ea,Tokens=ee}X=eo else break end end;return X end
...
C("`then` expected.")end;local ea=F(Q)
...
not U:ConsumeKeyword('do',X)then return C("`do` expected.")end;local ee=F(Q)if not
U:ConsumeKeyword('end',X)then C("`end` expected.")end
...
C("<ident> expected.")end;local Z=U:Get(X)
...
U:ConsumeKeyword('do',X)then C("`do` expected")end
local en=F(ee)
...
if not U:Is('Ident')then C("for variable expected.")end;et[#et+1]=ee:CreateLocal(U:Get(X).Data)end
...
C("`end` expected.")end
J={AstType='GenericForStatement',Scope=ee,VariableList=et,Generators=ea,Body=eo,Tokens=X}end elseif U:ConsumeKeyword('repeat',X)then local Z=F(Q)if
not U:ConsumeKeyword('until',X)then C("`until` expected.")end
...
if not
U:Is('Ident')then C("local var name expected")end;Z[#Z+1]=U:Get(X).Data end;local ee={}if U:ConsumeSymbol('=',X)then repeat ee[#ee+1]=M(Q)until
not U:ConsumeSymbol(',',X)end;for et,ea in
pairs(Z)do Z[et]=Q:CreateLocal(ea)end
J={AstType='LocalStatement',LocalList=Z,InitList=ee,Tokens=X}elseif U:ConsumeKeyword('function',X)then if not U:Is('Ident')then
C("Function name expected")end;local Z=U:Get(X).Data
local ee=Q:CreateLocal(Z)local et=V(Q,X)et.Name=ee;et.IsLocal=true;J=et else
C("local var or function def expected")end elseif U:ConsumeSymbol('::',X)then if not U:Is('Ident')then
C('Label name expected')end;local Z=U:Get(X).Data;if
not U:ConsumeSymbol('::',X)then C("`::` expected")end
...
if ee then Z[1]=et;while U:ConsumeSymbol(',',X)do Z[#Z+1]=M(Q)end end end
...
C("Can not assign to parenthesized expression, is not an lvalue")end;local ee={Z}while
U:ConsumeSymbol(',',X)do ee[#ee+1]=P(Q)end;if
not U:ConsumeSymbol('=',X)then C("`=` Expected.")end;local et={M(Q)}while
U:ConsumeSymbol(',',X)do et[#et+1]=M(Q)end
...
J={AstType='CallStatement',Expression=Z,Tokens=X}else C("Assignment Statement Expected")end end
if U:IsSymbol(';')then J.Semicolon=U:Get(J.Tokens)end;return J end
function F(Q)local J={}local X={Scope=c(Q),AstType='Statlist',Body=J,Tokens={}}
while not
I[U:Peek().Data]and not U:IsEof()do local Z=K(X.Scope)J[#J+1]=Z end
if U:IsEof()then local Z={}Z.AstType='Eof'Z.Tokens={U:Get()}J[#J+1]=Z end;return X end;return F(c())end;return{LexLua=D,ParseLua=L}end)
...
if not
(D=='_'or z[D]or x[D]or _[D])then return N..S else return N..H..S end elseif _[R]then
if D=='('then return N..S elseif E[D]then return N..S else return N..H..S end elseif R==''then return N..S else if D=='('then return N..H..S else return N..S end end end
...
W..H(U.Arguments[Y])
if Y~=#U.Arguments then W=W..","end end;W=W..")"elseif
...
if#
U.Arguments>0 then for Y=1,#U.Arguments do W=W..U.Arguments[Y].Name
if Y~=#
...
"="..H(P.Value)end;if Y~=#U.EntryList then W=W..","end end;W=W.."}"elseif U.AstType=='Parentheses'then
...
if U.AstType=='AssignmentStatement'then for M=1,#U.Lhs do C=C..H(U.Lhs[M])if M~=#
U.Lhs then C=C..","end end
if
#U.Rhs>0 then C=C.."="for M=1,#U.Rhs do C=C..H(U.Rhs[M])
if M~=#U.Rhs then C=C..","end end end elseif U.AstType=='CallStatement'then C=H(U.Expression)elseif
U.AstType=='LocalStatement'then C=C.."local "
for M=1,#U.LocalList do
C=C..U.LocalList[M].Name;if M~=#U.LocalList then C=C..","end end;if#U.InitList>0 then C=C.."="
for M=1,#U.InitList do
C=C..H(U.InitList[M])if M~=#U.InitList then C=C..","end end end elseif
...
C=D(C,S(F.Body))end;C=D(C,"end")elseif U.AstType=='WhileStatement'then
...
for M=1,#U.Arguments do C=D(C,H(U.Arguments[M]))if M~=#
...
M~=#U.Arguments then C=C..","elseif U.VarArg then C=C..",..."end end elseif U.VarArg then C=C.."..."end;C=C..")"C=D(C,S(U.Body))C=D(C,"end")elseif
U.AstType=='GenericForStatement'then U.Scope:ObfuscateLocals()C="for "for M=1,#U.VariableList do C=C..
U.VariableList[M].Name
if M~=#U.VariableList then C=C..","end end;C=C.." in"
for M=1,#U.Generators do
C=D(C,H(U.Generators[M]))if M~=#U.Generators then C=D(C,',')end end;C=D(C,"do")C=D(C,S(U.Body))C=D(C,"end")elseif
...
'Eof'then else
error("Unknown AST Type: "..U.AstType)end;R=R+#C;return C end
S=function(U)local C=''U.Scope:ObfuscateLocals()for M,F in pairs(U.Body)do
C=D(C,L(F),';')end;return C end;return S(N)end;local function O(N)local S=f.LexLua(N)t.refreshYield()S=f.ParseLua(S)
t.refreshYield()return A(S)end
...
L.close()end;return{JoinStatements=T,Minify=A,MinifyString=O,MinifyFile=I}end)
...
assert(A==#E,"Tables must be the same length")for O=1,A do j(_[O],E[O])end else j(_,E)end end):Description(
"Minifies '"..
fs.getName(_).."' into '"..fs.getName(E).."'"):Requires(_):Produces(E)end
function d.Runner:MinifyAll(z,_,E)z=z or"_minify"return
self:AddTask(z,{},x):Description("Minifies files"):Maps(
_ or"wild:*.lua",E or"wild:*.min.lua")end
i.Subscribe({"HowlFile","env"},function(z)z.Minify=j end)end
local y=e(function(j,...)local x={}function x:Add(_)
table.insert(self.include,self:_Parse(_))self.files=nil;return self end;function x:Remove(_)
table.insert(self.exclude,self:_Parse(_))self.files=nil;return self end
...
self:_Include("",T.Text)end end end;return self.files end
function x:_Include(_,E)if _~=""then
for A,E in pairs(self.exclude)do if E.Match(_)then return end end end
local T=fs.combine(self.path,_)
assert(fs.exists(T),"Cannot find path ".._)if fs.isDir(T)then for A,O in ipairs(fs.list(T))do
self:_Include(fs.combine(_,O),E)end elseif not E or E:match(_)then
self.files[_]=true end end
function x:_Parse(_)_=o.ParsePattern(_)local E=_.Text
if o.Type=="Normal"then
function _.Match(T)return E==T end else function _.Match(T)return T:match(E)end end;return _ end
local function z(_)return
setmetatable({path=_ or s.CurrentDirectory,include={},exclude={},startup='startup'},{__index=x})end
i.Subscribe({"HowlFile","env"},function(_)_.Files=z end)return{Files=x,Factory=z}end)
do
...
error('You must have a file called '..self.startup..' to be executed at runtime.')end;local A={}
for N,S in pairs(T)do
local H=fs.open(fs.combine(E,N),"r")local R=H.readAll()H.close()if _.minify and loadstring(R)then
R=w.MinifyString(R)end;local D=A
local L={N:match((N:gsub("[^/]+/?","([^/]+)/?")))}L[#L]=nil
for S,U in pairs(L)do local C=D[U]if not C then C={}D[U]=C end;D=C end;D[fs.getName(N)]=R end
local O=j.."local files = "..t.serialize(A)..
"\n"..string.format(x,self.startup,self.startup)if _.minify then O=w.MinifyString(O)end
local I=fs.open(fs.combine(s.CurrentDirectory,z),"w")I.write(O)I.close()end
function d.Runner:Compilr(z,_,E,T)return
self:AddTask(z,T,function()_:Compilr(E)end):Description("Combines multiple files using Compilr"):Produces(E)end end
...
if R then
E("Busted loading at "..H)local D=setfenv(R,getfenv())()D=D.api or D;if D.run then E(
"Busted found at "..H)return D end end end
local function I(H)if not x(H)then return end;if not z(H)then return O(H)end;local R
for D,L in ipairs(A)do R=j(H,L)if
x(R)then local U=O(R)if U then return U end end end end
...
local function S()
return
{cwd=s.CurrentDirectory,output='colorTerminal',seed=os.time(),verbose=o.IsVerbose(),root='spec',tags={},['exclude-tags']={},pattern='_spec',loaders={'lua'},helper=''}end
...
if U~=0 then
o.VerboseLog(messages)error("Not all tests passed")end end):Description("Runs tests")end end;local p=n.Options({...})local v=d.Factory()local b=p:Arguments()
i.Subscribe({"ArgParse","changed"},function(p)o.IsVerbose(
p:Get("verbose")or false)
v.ShowTime=p:Get"time"v.Traceback=p:Get"trace"if p:Get"help"then b={"help"}end end)
...
o.PrintColor(colors.white,"Now just run `Howl minify`!")end;error(k,0)end;s.CurrentDirectory=k
